...
const PlaylistView = () => {
  ...
  return (
    <div>
      {nowPlaying && <h2 className='text-white text-center my-10 mx-10 xl:mx-0'><span className='themed-color'>Now Playing</span>: {nowPlaying.current.title}</h2>}
      <div className='mb-2' style={{margin: "0 20%"}}>
        ...
        <Row>
          ...
          <Col xs={12} lg={6} xl={4} className='flex justify-center'>
        ...
        <ListGroup style={{ height: "70vh", overflowY: "scroll", width: "100%" }}>
          {
            queues.map((music, index) => (
              <ListGroupItem key={index} className='text-base text-left bg-grey ' active={nowPlaying && (index == queues.filter(music => music.queue_id === nowPlaying.current_queue_id)[0].index)}>
                <Row>
                  ...
                  <Col xs={1} className='flex justify-end cursor-default text-sm xl:text-xl xl:mr-1'>{secondFormatting(music.duration)}</Col>
                </Row>
              </ListGroupItem>
